...
function Comments(props) {
  ...
  //Function for the text change
  const handleChange = (event) => {
    setEditableComment(event.target.value);
  };

  //Function  for text change/new comment
  ...
  if (postComment.length === 200) {
    toast("Comment Should be less than 200 words");
  }
            ...
            <Grid container wrap="nowrap" spacing={2}>
              {!isEdit[i] ? (
                <Grid item xs zeroMinWidth>
                  ...
                  <p className={classes.comment}>{comment.comments}</p>
                  ...
                  {userData._id === comment.user._id && (
                    <p style={{ textAlign: "left", color: "gray" }}>
                      <Button
                        size="small"
                        onClick={() => handleEdit(comment, i)}
                        disabled={EditableComment !== "" ? true : false}
                      >
                        Edit
                      </Button>
                      <Button
